...
class CategoryListAPI(BaseRoutes):
    ...
    def get(self):
        category_data=self.service.list_categories()
...
class CategoryDetailsAPI(BaseRoutes):
    ...
    def patch(self,category_id: int):
        data= request.get_json()
        response=  self.service.update_category(category_id=category_id, data=data)
        return self.format_response(data=response)

    def delete(self,category_id: int):
        response= self.service.delete_category(category_id)
